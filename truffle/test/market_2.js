...
contract("Market Test 2", (accounts) => {
  ...
  it("should start auction", async () => {
    ...
    await market.auction({from:accounts[0], value: 3400});

    // Vedanta should have 1000 more
    assert.equal(parseInt(initialBalanceSuppliers[1]) + 1000, await web3.eth.getBalance(accounts[1]));     

    // MRF should have 1000 more
    assert.equal(parseInt(initialBalanceSuppliers[2]) + 1000, await web3.eth.getBalance(accounts[2]));
